function localize(ax,sig1,sig2,sig3,sig4,Fs)
...
contour(ax,X, Y, Z1, [0, 0], 'g-', 'LineWidth', 1.5);
hold (ax,'on');
contour(ax,X, Y, Z2, [0, 0], 'b-', 'LineWidth', 1.5);
contour(ax,X, Y, Z3, [0, 0], 'm-', 'LineWidth', 1.5);



% Plot microphone positions
plot(ax,[mic1(1), mic2(1), mic3(1), mic4(1)], [mic1(2), mic2(2), mic3(2), mic4(2)], 'ro', 'MarkerSize', 10, 'LineWidth', 2);

% Plot the estimated source point
plot(ax,center_x(1),center_x(2),'rx', 'MarkerSize', 10, 'LineWidth', 2);

% Add labels and legend
xlabel(ax,'X-coordinate');
ylabel(ax,'Y-coordinate');
title(ax,'Acoustic Triangulation');
legend(ax,'eq1', 'eq2', 'eq3', 'Microphones', 'Estimated Source');
...
% Optional: Display a message indicating that the data was appended
disp('Data appended to the file.');
