function localize_v2(sig1,sig2,sig3,sig4,Fs)
...
td34 = gcc(signal1,signal3,Fs)-td78
td14 = gcc(signal1,signal4,Fs)-td58


% Hyperbolas to solve
eq1 = @(x, y) td12 * 343 - (sqrt((x-mic1(1)).^2 + (y-mic1(2)).^2) - sqrt((x - mic2(1)).^2 + (y - mic2(2)).^2));
eq2 = @(x, y) td23 * 343 - (sqrt((x-mic2(1)).^2 + (y-mic2(2)).^2) - sqrt((x - mic3(1)).^2 + (y - mic3(2)).^2));
eq3 = @(x, y) td34 * 343 - (sqrt((x-mic3(1)).^2 + (y-mic3(2)).^2) - sqrt((x - mic4(1)).^2 + (y - mic4(2)).^2));
eq4 = @(x, y) td14 * 343 - (sqrt((x-mic1(1)).^2 + (y-mic1(2)).^2) - sqrt((x - mic4(1)).^2 + (y - mic4(2)).^2));
...
[x1, y1] = fsolve(@(xy) [eq1(xy(1), xy(2)), eq2(xy(1), xy(2)), eq3(xy(1), xy(2)), eq4(xy(1), xy(2))], initial_guess, options);
[x2, y2] = fsolve(@(xy) [eq1(xy(1), xy(2)), eq2(xy(1), xy(2)), eq3(xy(1), xy(2)), eq4(xy(1), xy(2))], [0.5, 0.8], options);
[x3, y3] = fsolve(@(xy) [eq1(xy(1), xy(2)), eq2(xy(1), xy(2)), eq3(xy(1), xy(2)), eq4(xy(1), xy(2))], [0.4, 0.25], options);
[x4, y4] = fsolve(@(xy) [eq1(xy(1), xy(2)), eq2(xy(1), xy(2)), eq3(xy(1), xy(2)), eq4(xy(1), xy(2))], [0.4, 0.25], options);


% Calculate the center of the triangle
center_x = (x1 + x2 + x3 + x4) / 4;
center_y = (y1 + y2 + y3 + y4) / 3;
...
contour(X, Y, Z4, [0, 0], 'y-', 'LineWidth', 1.5);
...
legend('eq1', 'eq2', 'eq3', 'eq4', 'Microphones', 'Estimated Source');
